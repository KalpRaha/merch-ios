...
class SMTabbar: UIScrollView {
    ...
    private func initLineLayer() {
        let y : CGFloat = self.linePosition == .bottom ? self.frame.height - self.lineHeight : 0 //self.lineYoffset
    ...
    @objc fileprivate func handleTap(_ sender : UIButton) {
       ...
       // page.goToSpecificPage(index: sender.tag, ofViewControllers: ReportingPagerViewController().subControllers)
    }
}
