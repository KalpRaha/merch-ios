...
class ItemsPOViewController: UIViewController {
    ...
    @IBOutlet weak var vendorEmailLbl: UILabel!
    ...
    override func viewDidLoad() {
        ...
        let tap2 = UITapGestureRecognizer(target: self, action: #selector(emailClick))
        emailLbl.addGestureRecognizer(tap2)
    ...
    func getResponseValues(list: Any) {
        ...
        email_number = details.po_number
        email_supplier = details.vendor_name
        
        let ref = details.reference
        
        if ref == "" || ref == "<null>" {
            refLbl.text = ""
        }
        else {
            refLbl.text = "Reference: \(ref)"
        }
    ...
    func getPOVariants() {
        ...
        for variant in smallVarList {
            ...
            let item = VariantPOModel(po: variant, isSelect: true, reqQty: poItem.required_qty,
                ...
                orderItem: poItem.id, status: poItem.recieved_status, pendingQty: poItem.pending_qty, check: "1")
...
//        }
    ...
    func updatePO() {
        ...
        if poSelectedVariants.count == 0 {
            ...
            for item in 0..<poSelectedVariants.count {
                ...
                if poSelectedVariants[item].status == "2" {
                    continue
                }
    ...
    @IBAction func threeBtnsClick(_ sender: UIButton) {
    ...
    @objc func emailClick() {
        ...
        let email_body = body
        let email_subject = "Purchase Order Details:\(email_number)"
        let email_name = id
        
        menu.isHidden = true
        
        ApiCalls.sharedCall.stockEmail(email_subject: email_subject,
                                       email_body: email_body, email_to: vendor_email,
...
//        }
    ...
    func emailHtml(htmlString: String) -> String {
        ...
        <td>
        <p style="margin-bottom:0px;"><b>Order Number</b> <span style="float:right;">\(email_number)</span></p>
        </td>
        </tr>
        <tr>
        <tr>
