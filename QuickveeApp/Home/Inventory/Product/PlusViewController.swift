...
class PlusViewController: UIViewController {
  ...
  @IBOutlet weak var missCancelBtn: UIButton!
  @IBOutlet weak var misscollection: UICollectionView!
  ...
  override func viewWillAppear(_ animated: Bool) {
      ...
      missCancelBtn.layer.cornerRadius = 10
      missCancelBtn.layer.borderColor = UIColor.black.cgColor
      missCancelBtn.layer.borderWidth = 1.0
      
      missView.layer.maskedCorners = [.layerMinXMinYCorner, .layerMaxXMinYCorner]
...
//      
    ...
    func saveProductInfo() {
        ...
        if !name.isEmpty {
            UserDefaults.standard.set(name, forKey: "Product_name")
            isValueSaved = true
        }
        else {
            UserDefaults.standard.set("", forKey: "Product_name")
            isValueSaved = true
        }
        
        if !description.isEmpty {
            UserDefaults.standard.set(description, forKey: "Product_description")
            isValueSaved = true
        }
        else {
            UserDefaults.standard.set("", forKey: "Product_description")
            isValueSaved = true
        }
        
        
        if !brand.isEmpty {
            UserDefaults.standard.set(brand, forKey: "Product_brand")
            isValueSaved = true
            print(isValueSaved)
        }
        else {
        ...
        if !collTag.isEmpty {
            UserDefaults.standard.set(collTag, forKey: "tagArray")
            isValueSaved = true
        }
        else {
        ...
        if !autoresult.isEmpty {
            print(autoresult)
            UserDefaults.standard.set(autoresult, forKey: "resultKey")
        } else {
            print("autoresult is empty")
        }
        
        
        if let encodedVariants = try? JSONEncoder().encode(variantsArray) {
            UserDefaults.standard.set(encodedVariants, forKey: "plus_variants_draft")
            isValueSaved = true
        }
        else {
            UserDefaults.standard.set([], forKey: "plus_variants_draft")

        }
        
        
        UserDefaults.standard.set(isValueSaved, forKey: "isDraftSaved")
    ...
    func loadSavedProduct() {
        ...
        var isValueSaved = false
           ...
           let name = UserDefaults.standard.string(forKey: "Product_name") ?? ""
           let description = UserDefaults.standard.string(forKey: "Product_description") ?? ""
           let brand = UserDefaults.standard.string(forKey: "Product_brand") ?? ""
           ...
           if !brand.isEmpty {
               ...
               DispatchQueue.main.async {
                   self.brandInnerView.isHidden = false
                   self.brandCloseBtn.isHidden = false
                   self.selectBrandLbl.text = ""
                   self.brandName.text = brand
               }
           } else {
               
               DispatchQueue.main.async {
                   self.brandInnerView.isHidden = true
                   self.brandCloseBtn.isHidden = true
                   self.selectBrandLbl.text = "Select Brand"
                   self.brandName.text = ""
               }
           }
...
//        }
        ...
        if let data = UserDefaults.standard.data(forKey: "variantAttributes"),
           let decoded = try? JSONDecoder().decode([InventoryOptions].self, from: data) {
            ...
            for i in 0..<productOptions.count {
                if i == 0 {
                    arrOptVl1 = productOptions[i].optionsvl1.components(separatedBy: ",")
                }
                else if i == 1 {
                    arrOptVl2 = productOptions[i].optionsvl2.components(separatedBy: ",")
                }
                else {
                    arrOptVl3 = productOptions[i].optionsvl3.components(separatedBy: ",")
                }
            }
        }
        
        
        if let savedResult = UserDefaults.standard.array(forKey: "resultKey") as? [String] {
            result = savedResult
            print(result)
        }
  ...
  func clearDraft() {
      ...
      print(UserDefaults.standard.bool(forKey: "isDraftSaved"))
      let keys = [
             "Product_name",
             "Product_description",
             "Product_brand",
          ...
          if textField == productField {
              ...
              if textField == cell.costPerItem {
                  ...
                  if cost_per_value == "" || cost_per_value == "0.00" {
                      ...
                      let priceText = cell.price.text ?? "0.00"
                      let costText = textField.text ?? "0.00"
                      let price = Double(priceText) ?? 0.00
                      let cost = Double(costText) ?? 0.00

                      let profit = price - cost
                      let margin = price != 0 ? (profit / price) * 100 : 0.0

                     
                      variantsArray[index.section].costperItem = String(format: "%.2f", cost)
                      ...
                      for variants in 0..<variantsArray.count {
                          if variantsArray[variants].costperItem == "0.00" {
                              variantsArray[variants].costperItem = String(format: "%.2f", cost)
...
extension PlusViewController {
    ...
    func dataExistsAndNonEmpty<T: Decodable>(_ key: String, as type: T.Type) -> Bool where T: Collection {
...
extension PlusViewController: UICollectionViewDelegate, UICollectionViewDataSource {
  ...
  func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
      ...
      if collectionView == catColl {
          ...
          let cell = misscollection.dequeueReusableCell(withReuseIdentifier: "miss", for: indexPath) as! MissUPCCollectionViewCell
          
          cell.missLbl.text = missVariants[indexPath.row]
